...
# Loading all data
CoCosV10I_16S     <- read_table('data/read_qc/trim_fix/Sample_statistics_CoCosV10I_16S_filtered.txt')
...
RSV5_16S          <- read_table('data/read_qc/trim_fix/Sample_statistics_RS21AUG_16S_filtered.txt')
...
#   filter(!grepl('NTC|WC_|DI_|BC_|EB_', file)) %>%
#   select(file, num_seqs, `Q30(%)`) %>% 
#   mutate(data_source = 'CoCos VII',
#          Sequencer = 'NextSeq 2000') -> CoCosV10II_16S
# 
# CoCosV10II_MiFish %>%
#   filter(!grepl('NTC|WC_|DI_|BC_|EB_', file)) %>%
#   select(file, num_seqs, `Q30(%)`) %>% 
#   mutate(data_source = 'CoCos VII',
#          sequencer = 'NextSeq 2000') -> CoCosV10II_MiFish
...
QC_plot <- plot_grid(number, QC30, labels = NULL, label_size = 12)
