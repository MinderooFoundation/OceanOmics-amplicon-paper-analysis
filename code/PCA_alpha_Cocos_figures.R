...
samples <- list()
samples$cki_ind <- readRDS(file = "data/phyloseq_objects/trim_fix/CoCosV10I_16S_phyloseq_nt_FALSE.rds")
samples$cki_pooled <- readRDS(file = "data/phyloseq_objects/trim_fix/CoCosV10I_16S_phyloseq_nt_TRUE.rds")
samples$cki_pseudo <- readRDS(file = "data/phyloseq_objects/trim_fix/CoCosV10I_16S_phyloseq_nt_pseudo.rds")

length(rownames(samples$cki_ind@otu_table)) #462
length(rownames(samples$cki_pseudo@otu_table)) #501
length(rownames(samples$cki_pooled@otu_table)) #575
...
length(rownames(samples$cki_ind@otu_table)) #312
length(rownames(samples$cki_pseudo@otu_table)) #404
length(rownames(samples$cki_pooled@otu_table)) #549
...
length(rownames(samples$cki_ind@otu_table)) #272
length(rownames(samples$cki_pseudo@otu_table)) #340
length(rownames(samples$cki_pooled@otu_table)) #470

saveRDS(samples$cki_ind, file = "data/phyloseq_objects/trim_fix/CoCosV10I_16S_phyloseq_nt_FALSE_decontaminated.rds")
saveRDS(samples$cki_pooled, file = "data/phyloseq_objects/trim_fix/CoCosV10I_16S_phyloseq_nt_TRUE_decontaminated.rds")
saveRDS(samples$cki_pseudo, file = "data/phyloseq_objects/trim_fix/CoCosV10I_16S_phyloseq_nt_pseudo_decontaminated.rds")
...
Cocos_alpha_ind <- data.frame(divnet_alpha_cki_ind %>% summary, sample_data(samples$cki_ind)) %>%
  mutate(Option = 'Indendent') %>%
  distinct(estimate, error, lower, upper, site_id, Option)
Cocos_alpha_pool <- data.frame(divnet_alpha_cki_pooled %>% summary, sample_data(samples$cki_pooled)) %>%
  mutate(Option = 'Pooled') %>%
  distinct(estimate, error, lower, upper, site_id, Option)

# Cocos_alpha <- rbind(Cocos_alpha_ind, Cocos_alpha_pool) %>% 
#   ggplot(aes(x = site_id, y = estimate, col = Option, group = Option)) + geom_point() + geom_segment(aes(x = .data[["site_id"]], xend = .data[["site_id"]], y = .data[["lower"]], yend = .data[["upper"]])) + guides(colour = FALSE) + xlab("Site identifier") + ylab("Shannon entropy estimate") + theme_bw()
# # add in legend
# legend <- get_legend(
#   Cocos_alpha +
#     guides(color = guide_legend(ncol = 1)) +
...
# )
# 
# plot_grid(Cocos_alpha, legend, rel_widths = c(2, .2), align = "hv", axis = "l")

Cocos_alpha <- rbind(Cocos_alpha_ind, Cocos_alpha_pool)
Cocos_alpha_plot <- ggplot(data = Cocos_alpha, aes(x = site_id, y = estimate, col = Option, group = Option)) + geom_point(position = position_dodge(0.5)) + geom_errorbar(aes(ymin = Cocos_alpha$lower, ymax = Cocos_alpha$upper), width = 0.2, position = position_dodge(0.5)) + guides(colour = FALSE) + xlab("Site identifier") + ylab("Shannon entropy estimate") + theme_bw()
Cocos_alpha_plot + scale_colour_viridis_d()
...
Cocos_alpha <- rbind(Cocos_alpha_ind, Cocos_alpha_pool)
Cocos_alpha
ggplot(Cocos_alpha, aes(site_id, estimate, group = Option, col=Option)) + geom_point() + geom_segment(aes(x = .data[["site_id"]], xend = .data[["site_id"]], y = .data[["lower"]], yend = .data[["upper"]])) + guides(colour = FALSE) + xlab("Site identifier") + ylab("Shannon entropy estimate") + theme_bw()
