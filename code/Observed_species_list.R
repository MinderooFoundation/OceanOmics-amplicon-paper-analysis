...
length(observed_species_list$species) # 1201
length(unique(observed_species_list$species)) # 453

observed_species_list %>% 
  distinct(species, checklist) %>%
  filter(checklist == FALSE)  -> species_false
length(species_false$species) # 50

observed_species_list %>% 
  distinct(species, checklist, Mode) %>%
  filter(checklist == FALSE) %>%
  filter(Mode == "Pooled")  -> species_false_pooled
length(species_false_pooled$species) # 47

observed_species_list %>% 
  distinct(species, checklist) %>%
  filter(checklist == TRUE)  -> species_true
length(species_true$species) # 403
## manually changed a few species which should have been in there
...
# 1 Bolinichthys distofax  Pooled FALSE    - potentially real false hit
# 2 Kali indica            Pooled FALSE - potentially real false hit    
...
cocos_diff_true <- subset(observed_species_list, species %in% diff) %>%
  ...
  filter(Dataset == "Cocos Islands") %>%
  distinct(species, Mode, checklist) 
length(unique(cocos_diff_true$species)) #8
length(unique(cocos_diff_false$species)) #3
...
# 2 Pseudojuloides cerasinus       Pooled FALSE  - potentially real false hit  
...
rs_diff_true <- subset(observed_species_list, species %in% diff) %>% 
  ...
  filter(Dataset == "Rowley Shoals") %>%
  distinct(species, Mode, checklist)
length(unique(rs_diff_false$species)) #6
length(unique(rs_diff_true$species)) #98
...
nw_diff_true <- subset(observed_species_list, species %in% diff) %>% 
  ...
  filter(Dataset == "North-west") %>%
  distinct(species, Mode, checklist)
length(unique(nw_diff_false$species)) #13
length(unique(nw_diff_true$species)) #78
...
true_hits <- rbind(cocos_diff_true, rs_diff_true, nw_diff_true) %>%
  distinct(species)
length(unique(true_hits$species)) #175

false_hits <- rbind(cocos_diff_false, rs_diff_false, nw_diff_false) %>%
  distinct(species)
length(unique(false_hits$species)) #22 - 3 = 19
