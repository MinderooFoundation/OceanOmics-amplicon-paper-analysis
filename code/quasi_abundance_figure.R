...
length(cocos_species_pooled$Abundance) #373
...
nw_species_ind <- nw_species %>% 
  distinct(species, Abundance, Option) %>%
  filter(Option == "Independent")
length(nw_species_ind$Abundance) #160
...
nw_species_pooled <- nw_species %>% 
  distinct(species, Abundance, Option) %>%
  filter(Option == "Pooled")
length(nw_species_pooled$Abundance) #367
...
rs_species_ind <- rs_species %>% 
  distinct(species, Abundance, Option) %>%
  filter(Option == "Independent")
length(rs_species_ind$Abundance) #673
...
rs_species_pooled <- rs_species %>% 
  distinct(species, Abundance, Option) %>%
  filter(Option == "Pooled")
length(rs_species_pooled$Abundance) #1226
