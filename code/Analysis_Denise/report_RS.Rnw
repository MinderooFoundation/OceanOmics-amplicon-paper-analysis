...
load("reports/amplicon_pipeline_RS.RData")
@
...
Read in the metadata and processed sequencing data and filter out ASVs that are not observed in at least 3 samples. This minimal filtering removes 373/1073 ASVs for the \hltexttt{independent}, 16/1009 ASVs for the \hltexttt{pooled} and 269/1172 ASVs for the \hltexttt{pseudo} processing.
...
metadata <- read.csv(file = "data/processed/RSV5_metadata.csv")

samples <- list()
samples$rs_ind <- readRDS(file = "data/processed/RSV5_16S_FALSE_phyloseq_nt.rds")
samples$rs_pooled <- readRDS(file = "data/processed/RSV5_16S_TRUE_phyloseq_nt.rds")
samples$rs_pseudo <- readRDS(file = "data/processed/RSV5_16S_pseudo_phyloseq_nt.rds")
...
Run decontam on the samples using the water control samples and remove any contaminating ASVs. Figure \ref{decontam} shows the estimates from decontam, which are the probability of an ASV not being a contaminant. It can be seen that using the DADA2 \hltexttt{pooled} and \hltexttt{pseudo} processing (Figure \ref{decontam}) results in ASVs with higher prevalence (i.e. being called across more samples) when compared to the \hltexttt{independent} approach. A threshold needs to be chosen to define an ASV as a contaminant. The authors state that it is usually straightforward to choose a threshold from the plot, as the probabilities typically separate into a bimodal distribution of ``low'' and ``high'' scores. Our data does not show a clear bimodal distribution, however a threhold of 0.5 looks suitable for filtering out potential contaminants, as the distribution looks similar to the second example discussed \href{https://github.com/benjjneb/decontam/issues/41}{here}. In this example, there are a number of small modes at lower probabilities that are likely contaminants, so multiple thresholds of increasing stringency can be chosen. Using a threshold of 0.5, removes 64/700 ASVs for the \hltexttt{independent}, 130/993 ASVs for the \hltexttt{pooled} and 142/903 ASVs for the \hltexttt{pseudo} processed samples. 

After removing contaminant ASVs, filter out samples that have less than 1,000 reads. There are no samples that have less than 1,000 reads. The final numbers for downstream analysis are 636 ASVs and 106 samples for the \hltexttt{independent}, 863 ASVs and 106 samples for the \hltexttt{pooled} and 761 ASVs and 106 samples for the \hltexttt{pseudo} processed samples.
...
saveRDS(samples$rs_ind, file = "data/processed/decontaminated/RSV5_16S_FALSE_phyloseq_nt_decontaminated.rds")
saveRDS(samples$rs_pooled, file = "data/processed/decontaminated/RSV5_16S_TRUE_phyloseq_nt_decontaminated.rds")
saveRDS(samples$rs_pseudo, file = "data/processed/decontaminated/RSV5_16S_pseudo_phyloseq_nt_decontaminated.rds")
@
...
plot_heatmap(samples$rs_ind, taxa.label = "LCA_ASV", low = "lavenderblush", high = "red", na.value = "white", max.label = nrow(otu_table(samples$rs_ind)), title = "DADA2 independent processing") + theme(axis.text.x = element_text(size = 4), axis.text.y = element_text(size = 1))
@
\caption{Heatmap of Rowley Shoals samples, where rows and columns are organised using NMDS ordination of Bray-Curtis dissimilarities. Zero counts are coloured white and abundances are shown on a log\textsubscript{4} scale.}
\label{hm1}
\end{figure}

\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=12, fig.width=9>>=
plot_heatmap(samples$rs_pooled, taxa.label = "LCA_ASV", low = "lavenderblush", high = "red", na.value = "white", max.label = nrow(otu_table(samples$rs_pooled)), title = "DADA2 pooled processing") + theme(axis.text.x = element_text(size = 4), axis.text.y = element_text(size = 1))
@
\caption{Heatmap of Rowley Shoals samples, where rows and columns are organised using NMDS ordination of Bray-Curtis dissimilarities. Zero counts are coloured white and abundances are shown on a log\textsubscript{4} scale.}
\label{hm2}
\end{figure}
...
\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=12, fig.width=9>>=
plot_heatmap(samples$rs_pseudo, taxa.label = "LCA_ASV", low = "lavenderblush", high = "red", na.value = "white", max.label = nrow(otu_table(samples$rs_pseudo)), title = "DADA2 pseudo processing") + theme(axis.text.x = element_text(size = 4), axis.text.y = element_text(size = 1))
@
\caption{Heatmap of Rowley Shoals samples, where rows and columns are organised using NMDS ordination of Bray-Curtis dissimilarities. Zero counts are coloured white and abundances are shown on a log\textsubscript{4} scale.}
\label{hm3}
\end{figure}
...
plot(pca_biplot_unlabelled(pca_data = samples_pca$rs_ind, phyloseq = samples$rs_ind, size = 3, colour = "Atoll", colour_label = "Atoll", shape = "Environment", shape_label = "Environment", title = "DADA2 independent processing"))
@
\caption{Covariance biplot of Rowley Shoals samples. Samples are colour coded by atoll and shapes indicate the environment. ASVs are labelled in grey with last common ancestor (LCA) and ASV identifier and note some labels are omitted for readability.}
\end{figure}

\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=10, fig.width=10>>=
plot(pca_biplot_unlabelled(pca_data = samples_pca$rs_pooled, phyloseq = samples$rs_pooled, size = 3, colour = "Atoll", colour_label = "Atoll", shape = "Environment", shape_label = "Environment", title = "DADA2 pooled processing"))
@
\caption{Covariance biplot of Rowley Shoals samples. Samples are colour coded by atoll and shapes indicate the environment. ASVs are labelled in grey with last common ancestor (LCA) and ASV identifier and note some labels are omitted for readability.}
\end{figure}
...
\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=10, fig.width=10>>=
plot(pca_biplot_unlabelled(pca_data = samples_pca$rs_pseudo, phyloseq = samples$rs_pseudo, size = 3, colour = "Atoll", colour_label = "Atoll", shape = "Environment", shape_label = "Environment", title = "DADA2 pseudo processing"))
@
\caption{Covariance biplot of Rowley Shoals samples. Samples are colour coded by atoll and shapes indicate the environment. ASVs are labelled in grey with last common ancestor (LCA) and ASV identifier and note some labels are omitted for readability.}
\end{figure}
...
DivNet estimates diversity indices using compositional data models \citep{pmid32432696}. DivNet requires a reference taxon, against which other abundances are to be compared. It does this automatically provided that there are some taxon that are observed in all samples. This was not the case for the other two datasets included in this study, so the reference taxon had to be chosen manually. It was suggested \href{https://github.com/adw96/DivNet/issues/14}{here} that the taxon with median abundance across all samples is good choice and I implemented this suggestion. Estimates are believed to be very robust to choice of reference taxon, however it is probably worth checking sensitivity to choice of reference taxon. Though this dataset did have ASVs that were observed across all samples, I have chosen the ASV with median abundance to match the way DivNet was run on the other two datasets.
...
\begin{figure}
<<echo=FALSE>>=
data.frame(divnet_alpha$rs_ind$shannon %>% summary, sample_data(samples$rs_ind)) %>%
...
@
\caption{DivNet estimates of Shannon entropy and associated confidence interval for the Rowley Shoals sites, with colour colour coding by Environment. Samples were processed using the DADA2 \hltexttt{independent} approach.}
\end{figure}

\begin{figure}
<<echo=FALSE>>=
data.frame(divnet_alpha$rs_pooled$shannon %>% summary, sample_data(samples$rs_pooled)) %>%
  distinct(estimate, error, lower, upper, Site, Environment) %>%
  ggplot(aes(x = Site, y = estimate, col = Environment)) + geom_point() + geom_segment(aes(x = .data[["Site"]], xend = .data[["Site"]], y = .data[["lower"]], yend = .data[["upper"]])) + xlab("Site") + ylab("Shannon entropy estimate") + theme_bw() + theme(axis.text.x = element_text(angle=90))
@
\caption{DivNet estimates of Shannon entropy and associated confidence interval for the Rowley Shoals sites, with colour colour coding by Environment. Samples were processed using the DADA2 \hltexttt{pooled} approach.}
\end{figure}

\begin{figure}
<<echo=FALSE>>=
data.frame(divnet_alpha$rs_pseudo$shannon %>% summary, sample_data(samples$rs_pseudo)) %>%
  distinct(estimate, error, lower, upper, Site, Environment) %>%
  ggplot(aes(x = Site, y = estimate, col = Environment)) + geom_point() + geom_segment(aes(x = .data[["Site"]], xend = .data[["Site"]], y = .data[["lower"]], yend = .data[["upper"]])) + xlab("Site") + ylab("Shannon entropy estimate") + theme_bw() + theme(axis.text.x = element_text(angle=90))
@
\caption{DivNet estimates of Shannon entropy and associated confidence interval for the Rowley Shoals sites, with colour colour coding by Environment. Samples were processed using the DADA2 \hltexttt{pseudo} approach.}
\end{figure}

<<echo=FALSE, eval=FALSE>>=
save.image("reports/amplicon_pipeline_RS.RData", compress = FALSE)
@
