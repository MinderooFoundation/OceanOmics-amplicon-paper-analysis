...
load("reports/amplicon_pipeline_West.RData")
@
...
Read in the processed sequencing data and add metadata from supplementary table 1 of the publication \citep{ddi.13228}. Filter out ASVs that are not observed in at least 3 samples. This minimal filtering removes 288/507 ASVs for the \hltexttt{independent}, 75/544 ASVs for the \hltexttt{pooled} and 253/509 ASVs for the \hltexttt{pseudo} processing.
...
samples <- list()
samples$west_ind <- readRDS(file = "data/processed/Pool_FALSE_KWEST_16S_16S_phyloseq_nt.rds")
samples$west_pooled <- readRDS(file = "data/processed/Pool_TRUE_KWEST_16S_16S_phyloseq_nt.rds")
samples$west_pseudo <- readRDS(file = "data/processed/Pool_pseudo_KWEST_16S_16S_phyloseq_nt.rds")
...
Frequency-based contaminant identification relies on auxiliary DNA quantitation data and prevalence-based contaminant identification relies on sequenced negative controls. Decontam is not intended to detect cross-contamination. For this dataset, negative controls have been sequenced, but not DNA quantitation data, so we will use prevalence-based contaminant identification. At least five negative controls are recommended for prevalence-based contaminant identification, but more than this will result in more accurate results. Further recommendations under GitHub issues are to decontaminate all data in a single sequencing batch together, using all applicable controls for increased power.

Run decontam on the samples using the extraction blank and filtration control samples and remove any contaminating ASVs. Section S2 of the supplementary information of the West et al. publication states that they used both types of control samples to filter ASVs. Figure \ref{decontam} shows the estimates from decontam, which are the probability of an ASV not being a contaminant. A threshold needs to be chosen to define an ASV as a contaminant. The authors state that it is usually straightforward to choose a threshold from the plot, as the probabilities typically separate into a bimodal distribution of ``low'' and ``high'' scores (see some examples \href{https://github.com/benjjneb/decontam/issues/41}{here}). To choose a threshold, I focussed on the \hltexttt{pooled} data and chose the point (probability=0.3) where there is an increase in the number of prevalent ASVs (i.e. observed in \textgreater 10 samples). Using this threshold, removes 46/219 ASVs for the \hltexttt{independent}, 36/469 ASVs for the \hltexttt{pooled} and 44/256 ASVs for the \hltexttt{pseudo} processed samples. 

After removing contaminant ASVs, filter out samples that have less than 1,000 reads. This filtering removes 11/284 samples for the \hltexttt{independent}, 8/284 samples for the \hltexttt{pooled} and 20/284 samples for the \hltexttt{pseudo} processed samples. The final numbers for downstream analysis are 173 ASVs and 273 samples for the \hltexttt{independent}, 433 ASVs and 276 samples for the \hltexttt{pooled} and 212 ASVs and 264 samples for the \hltexttt{pseudo} processed samples.
...
saveRDS(samples$west_ind, file = "data/processed/decontaminated/Pool_FALSE_KWEST_16S_phyloseq_nt_decontaminated.rds")
saveRDS(samples$west_pooled, file = "data/processed/decontaminated/Pool_TRUE_KWEST_16S_phyloseq_nt_decontaminated.rds")
saveRDS(samples$west_pseudo, file = "data/processed/decontaminated/Pool_pseudo_KWEST_16S_phyloseq_nt_decontaminated.rds")
@

\begin{figure}
<<echo=FALSE>>=
p1 <- ggplot(samples_contam$west_ind, aes(x = p, fill = Prevalence)) + geom_histogram(colour = "black", breaks = seq(from = 0, to = 1, by = 0.05), alpha = 0.5) + geom_vline(xintercept = 0.3, colour = "red") + xlab("Probability") + ylab("Count") + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(expand = c(0, 0))
p2 <- ggplot(samples_contam$west_pooled, aes(x = p, fill = Prevalence)) + geom_histogram(colour = "black", breaks = seq(from = 0, to = 1, by = 0.05), alpha = 0.5) + geom_vline(xintercept = 0.3, colour = "red") + xlab("Probability") + ylab("Count") + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(expand = c(0, 0))
p3 <- ggplot(samples_contam$west_pseudo, aes(x = p, fill = Prevalence)) + geom_histogram(colour = "black", breaks = seq(from = 0, to = 1, by = 0.05), alpha = 0.5) + geom_vline(xintercept = 0.3, colour = "red") + xlab("Probability") + ylab("Count") + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(expand = c(0, 0))

p1 + p2 + p3 + plot_layout(guides = "collect", ncol = 2) + plot_annotation(tag_levels = "A")
rm(p1, p2, p3)
@
\caption{Estimated probabilities from decontam of whether ASV is not a contaminant for samples processed using DADA2 \hltexttt{independent} (A),  \hltexttt{pooled} (B) and \hltexttt{pseudo} (C) processing. Histogram bars are colour coded by prevalence of the ASV across all samples. The vertical red line indicates the filtering threshold of 0.3 and this will remove 46/219, 36/469 and 44/256 ASVs from analysis respectively.}
\label{decontam}
\end{figure}
...
plot_heatmap(samples$west_ind, taxa.label = "LCA_ASV", low = "lavenderblush", high = "red", na.value = "white", max.label = nrow(otu_table(samples$west_ind)), title = "DADA2 independent processing") + theme(axis.text.x = element_text(size = 2), axis.text.y = element_text(size = 5))
@
\caption{Heatmap of samples collected in the tropical northwest by West et al., where rows and columns are organised using NMDS ordination of Bray-Curtis dissimilarities. Zero counts are coloured white and abundances are shown on a log\textsubscript{4} scale.}
\label{hm1}
\end{figure}

\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=12, fig.width=9>>=
plot_heatmap(samples$west_pooled, taxa.label = "LCA_ASV", low = "lavenderblush", high = "red", na.value = "white", max.label = nrow(otu_table(samples$west_pooled)), title = "DADA2 pooled processing") + theme(axis.text.x = element_text(size = 2), axis.text.y = element_text(size = 2))
@
\caption{Heatmap of samples collected in the tropical northwest by West et al., where rows and columns are organised using NMDS ordination of Bray-Curtis dissimilarities. Zero counts are coloured white and abundances are shown on a log\textsubscript{4} scale.}
\label{hm2}
\end{figure}
...
\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=12, fig.width=9>>=
plot_heatmap(samples$west_pseudo, taxa.label = "LCA_ASV", low = "lavenderblush", high = "red", na.value = "white", max.label = nrow(otu_table(samples$west_pseudo)), title = "DADA2 pseudo processing") + theme(axis.text.x = element_text(size = 2), axis.text.y = element_text(size = 4))
@
\caption{Heatmap of samples collected in the tropical northwest by West et al, where rows and columns are organised using NMDS ordination of Bray-Curtis dissimilarities. Zero counts are coloured white and abundances are shown on a log\textsubscript{4} scale.}
\label{hm3}
\end{figure}
...
plot(pca_biplot_unlabelled(pca_data = samples_pca$west_ind, phyloseq = samples$west_ind, size = 3, colour = "Subregion", colour_label = "Sub region", shape = "Habitat", shape_label = "Habitat", title = "DADA2 independent processing"))
@
\caption{Covariance biplot of tropical northwest samples collected by West et al.. Samples are colour coded by sub region and shape indicates habitat. ASVs are labelled in grey with last common ancestor (LCA) and ASV identifier and note some labels are omitted for readability.}
\end{figure}

\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=10, fig.width=10>>=
plot(pca_biplot_unlabelled(pca_data = samples_pca$west_pooled, phyloseq = samples$west_pooled, size = 3, colour = "Subregion", colour_label = "Sub region", shape = "Habitat", shape_label = "Habitat", title = "DADA2 pooled processing"))
@
\caption{Covariance biplot of tropical northwest samples collected by West et al.. Samples are colour coded by sub region and shape indicates habitat. ASVs are labelled in grey with last common ancestor (LCA) and ASV identifier and note some labels are omitted for readability.}
\end{figure}
...
\begin{figure}
<<echo=FALSE, warning=FALSE, fig.height=10, fig.width=10>>=
plot(pca_biplot_unlabelled(pca_data = samples_pca$west_pseudo, phyloseq = samples$west_pseudo, size = 3, colour = "Subregion", colour_label = "Sub region", shape = "Habitat", shape_label = "Habitat", title = "DADA2 pseudo processing"))
@
\caption{Covariance biplot of tropical northwest samples collected by West et al.. Samples are colour coded by sub region and shape indicates habitat. ASVs are labelled in grey with last common ancestor (LCA) and ASV identifier and note some labels are omitted for readability.}
\end{figure}
...
data.frame(divnet_alpha$west_ind$shannon %>% summary, sample_data(samples$west_ind)) %>%
...
@
\caption{DivNet estimates of Shannon entropy and associated confidence interval for tropical northwest samples collected by West et al., with colour coding by habitat. Samples were processed using the DADA2 \hltexttt{independent} approach.}
\end{figure}

\begin{figure}
<<echo=FALSE, fig.height=7, fig.width=12>>=
data.frame(divnet_alpha$west_pooled$shannon %>% summary, sample_data(samples$west_pooled)) %>%
  distinct(estimate, error, lower, upper, Site, Habitat) %>%
  ggplot(aes(x = Site, y = estimate, col = Habitat)) + geom_point() + geom_segment(aes(x = .data[["Site"]], xend = .data[["Site"]], y = .data[["lower"]], yend = .data[["upper"]])) + xlab("Site") + ylab("Shannon entropy estimate") + theme_bw() + theme(axis.text.x = element_text(angle=90, hjust = 1))
@
\caption{DivNet estimates of Shannon entropy and associated confidence interval for tropical northwest samples collected by West et al., with colour coding by habitat. Samples were processed using the DADA2 \hltexttt{pooled} approach.}
\end{figure}

\begin{figure}
<<echo=FALSE, fig.height=7, fig.width=12>>=
data.frame(divnet_alpha$west_pseudo$shannon %>% summary, sample_data(samples$west_pseudo)) %>%
  distinct(estimate, error, lower, upper, Site, Habitat) %>%
  ggplot(aes(x = Site, y = estimate, col = Habitat)) + geom_point() + geom_segment(aes(x = .data[["Site"]], xend = .data[["Site"]], y = .data[["lower"]], yend = .data[["upper"]])) + xlab("Site") + ylab("Shannon entropy estimate") + theme_bw() + theme(axis.text.x = element_text(angle=90, hjust = 1))
@
\caption{DivNet estimates of Shannon entropy and associated confidence interval for tropical northwest samples collected by West et al., with colour coding by habitat. Samples were processed using the DADA2 \hltexttt{pseudo} approach.}
\end{figure}

\end{landscape}

<<echo=FALSE, eval=FALSE>>=
save.image("reports/amplicon_pipeline_West.RData", compress = FALSE)
@
