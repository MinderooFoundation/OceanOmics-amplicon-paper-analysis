...
samples <- unlist(lapply(list.files('fastq/'), function(x) str_remove(x, ".fastq")))
samples <- c(samples[samples != "Controls"], unlist(lapply(list.files('fastq/Controls'), function(x) str_remove(x, ".fastq"))))
...
tibble(`Sample ID` = samples,
       assay = '16S',
       site = sites) %>%
  write_csv('metadata/KWest_metadata.csv')
